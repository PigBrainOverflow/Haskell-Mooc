...
buildList start count end = buildList' [end] start count

buildList' :: [Int] -> Int -> Int -> [Int]
buildList' xs start count
    | count <= 0 = xs
    | otherwise = buildList' (start : xs) start (count - 1)
...
sums :: Int -> [Int]
sums i = reverse (sums' [1] 2 i)

sums' :: [Int] -> Int -> Int -> [Int]
sums' (x : xs) i max
    | i <= max = sums' (x + i : x : xs) (i + 1) max
    | otherwise = (x : xs)
...
sumsOf' [] sums _ = sums
sumsOf' (x : xs) sums acc = sumsOf' xs (x + acc : sums) (x + acc)
