...
countValid = foldr (\ isvalid cnt -> if isvalid then (cnt + 1) else cnt) 0 . map f
  where
    f s = (s !! 2 == s !! 4) || (s !! 3 == s !! 5)
...
compose xs ys = map (\ (Just x) -> x) (filter isJust (map f xs))
  where
    f (x, y)
      = case mc of
        Nothing -> Nothing
        Just z -> Just (x, z)
      where mc = lookup y ys
...
permute is original = permute' (inversePermutation is) original []

permute' :: Permutation -> [a] -> [a] -> [a]
permute' (i : is) original new = original !! i : permute' is original new
permute' _ _ new = new

inversePermutation :: [Int] -> [Int]
inversePermutation perm = map snd . sortOn fst $ zip perm [0 ..]
