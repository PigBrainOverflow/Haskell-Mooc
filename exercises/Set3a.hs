...
firstHalf s = take half_len s
    where
        len = length s
        half_len = if even len then len `div` 2 else len `div` 2 + 1
...
powers :: Int -> Int -> [Int]
powers k max = reverse $ powers' [] k max

powers' :: [Int] -> Int -> Int -> [Int]
powers' [] k max = powers' [1] k max
powers' (x : xs) k max
    | x' <= max = powers' (x' : x : xs) k max
    | otherwise = (x : xs)
    where
        x' = x * k
...
joinToLength len ss = [s1 ++ s2 | s1 <- ss, s2 <- ss, length s1 + length s2 == len]
...
multiCompose' :: [a -> a] -> a -> a
multiCompose' [] = id
multiCompose' (f : fs) = multiCompose' fs . f
...
interpreter' (x, y) (c : cs)
    | c == "up" = interpreter' (x, y + 1) cs
    | c == "down" = interpreter' (x, y - 1) cs
    | c == "left" = interpreter' (x - 1, y) cs
    | c == "right" = interpreter' (x + 1, y) cs
    | c == "printX" = show x : interpreter' (x, y) cs
    | c == "printY" = show y : interpreter' (x, y) cs