...
allEqual [] = True
allEqual (x : xs) = foldr (&&) True (map (== x) xs)
...
middle x y z
    | xy `xor` xz = x
    | xy = if y < z then y else z
    | otherwise = if y < z then z else y
    where
        xy = x < y
        xz = x < z
...
longest :: (Ord a) => [[a]] -> [a]
longest ls = maximumBy myListCmp ls
    where
        myListCmp :: (Ord a) => [a] -> [a] -> Ordering
        myListCmp xs ys
            | cmplxly /= EQ = cmplxly
            | otherwise = compare (head ys) (head xs)
            where
                lx = length xs
                ly = length ys
                cmplxly = compare lx ly
...
incrementKey key xs = map (\ (x, y) -> if x == key then (x, y + 1) else (x, y)) xs
...
transfer from to amount bank
    | amount < 0 || from_account == Nothing || to_account == Nothing = bank
    | otherwise = case from_account of
        Just x -> if x < amount then bank else Map.adjust (\balance -> balance - amount) from $ Map.adjust (\balance -> balance + amount) to bank
    where
        from_account = Map.lookup from bank
        to_account = Map.lookup to bank
...
maxIndex arr
    = fst $ maximumBy myTupleCmp (assocs arr)
    where
        myTupleCmp :: (Ix i, Ord a) => (i, a) -> (i, a) -> Ordering
        myTupleCmp t1 t2 = compare (snd t1) (snd t2)
