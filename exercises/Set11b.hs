...
hSelectLines = hSelectLines' 1

hSelectLines' :: Int -> Handle -> [Int] -> IO [String]
hSelectLines' lineno h linenos = do
    iseof <- hIsEOF h
    if iseof
    then return []
    else do
        line <- hGetLine h
        if lineno `elem` linenos
        then do
            rest <- hSelectLines' (lineno + 1) h linenos 
            return $ line : rest
        else do
            rest <- hSelectLines' (lineno + 1) h linenos
            return rest
...
interact' f state = do
    line <- getLine
    let (isend, msg, state') = f (line, state)
    putStrLn msg
    if isend
