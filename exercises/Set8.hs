...
dotAndLine :: Picture
dotAndLine = Picture dotAndLineFunc

dotAndLineFunc :: Coord -> Color
dotAndLineFunc (Coord 3 4) = white
dotAndLineFunc (Coord _ 8) = pink
dotAndLineFunc _ = black
...
rectangle x0 y0 w h = Shape shape
    where
        shape (Coord x y) = x >= x0 && x < x0 + w && y >= y0 && y < y0 + h
...
paintSolid color (Shape s) (Picture p) = Picture p'
    where p' coord = if s coord then color else p coord
...
paint (Picture pattern) (Shape shape) (Picture base) = Picture p
    where
        p coord = if shape coord then pattern coord else base coord
...
instance Transform Fill where
    apply (Fill color) (Picture p) = Picture np
        where np _ = color
...
instance Transform Flip where
    ...
    apply FlipX (Picture p) = Picture np
        where np (Coord x y) = p $ Coord (-x) y
    apply FlipY (Picture p) = Picture np
        where np (Coord x y) = p $ Coord x (-y)
...
blur :: (Color, Color, Color, Color, Color) -> Color
blur ((Color r1 g1 b1), (Color r2 g2 b2), (Color r3 g3 b3), (Color r4 g4 b4), (Color r5 g5 b5)) = Color r g b
    where
        r = (r1 + r2 + r3 + r4 + r5) `div` 5
        g = (g1 + g2 + g3 + g4 + g5) `div` 5
        b = (b1 + b2 + b3 + b4 + b5) `div` 5

instance Transform Blur where
    apply Blur (Picture p) = Picture np
        where np (Coord x y) = blur ((p $ Coord x y), (p $ Coord (x - 1) y), (p $ Coord (x + 1) y), (p $ Coord x (y - 1)), (p $ Coord x (y + 1)))
...
instance Transform BlurMany where
    ...
    apply (BlurMany times) p = apply (BlurMany (times - 1)) (apply Blur p)
