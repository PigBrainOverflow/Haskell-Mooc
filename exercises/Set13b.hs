...
ifM opBool opThen opElse = do
  pred <- opBool
  if pred then opThen else opElse
...
findSum2 ks ns = [(x, y, n) | n <- ns, x <- ks, y <- ks, x + y == n]
...
allSums :: [Int] -> [Int]
allSums (x : xs) = allSums xs >>= (\ sum -> [sum, x + sum])
allSums _ = [0]
...
f1 k acc x = if acc' <= k then return acc' else Nothing
  where acc' = acc + x
...
instance Functor SL where
  -- implement fmap
  fmap f (SL slf) = SL slf'
    where
      slf' s = (f res', state', msgs')  -- just adjust the result
        where (res', state', msgs') = slf s
...
instance Monad SL where
  ...
  (SL slf) >>= f = SL slf''
    where
      slf'' s = (res'', state'', msgs' ++ msgs'')
        where
          (res', state', msgs') = slf s
          SL slf' = f res'
          (res'', state'', msgs'') = slf' state'
