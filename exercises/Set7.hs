...
member _ (Set []) = False
member target (Set (x : xs))
    | target > x = member target (Set xs)
    | target == x = True
    | otherwise = False

push :: a -> Set a -> Set a
push target (Set xs) = Set (target : xs)

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add target (Set []) = Set [target]
add target (Set (x : xs))
    | target < x = Set (target : x : xs)
    | target == x = Set (x : xs)
    | otherwise = push x $ add target (Set xs)
...
data State = Start | Error | Finished | WithEggs | WithFlour | WithSugar | WithAll | Mixed
    deriving (Eq, Show)
...
step WithFlour AddSugar = WithAll
step WithSugar AddFlour = WithAll
...
average xs = sum / fromIntegral cnt
    where
        (sum, cnt) = average' xs

average' :: Fractional a => NonEmpty a -> (a, Int)
average' = foldr (\ x (sum, cnt) -> (sum + x, cnt + 1)) (0, 0)
...
passwordAllowed s (ContainsSome cs) = getAny $ mconcat $ map (\ c -> Any $ elem c cs) s
...
data Arithmetic = Literal Integer | Add Arithmetic Arithmetic | Mul Arithmetic Arithmetic | ParseError
    deriving Show
