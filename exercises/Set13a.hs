...
split s
  | length sur < 2 = Nothing
  | otherwise = Just (for, tail sur)
  where
    for = takeWhile (/= ' ') s
    sur = dropWhile (/= ' ') s
...
checkCapitals (for, sur)
  | isAsciiUpper (head for) && isAsciiUpper (head sur) = Just (for, sur)
  | otherwise = Nothing
...
countAndLog pred = foldM f 0
  where
    f cnt x = if pred x then Logger [show x] (cnt + 1) else return cnt
...
balance accountName = BankOp f
  where
    f (Bank m) = case Map.lookup accountName m of
      Nothing -> (0, Bank m)
      Just b -> (b, Bank m)
...
-- implicitly pass bank state to next monad
...
paren c
  ...
  | otherwise = f2
  where
    f1 = modify (\ x -> if x < 0 then -1 else x + 1)
    f2 = modify (\ x -> if x < 0 then -1 else x - 1)
...
count x = do
  old <- get
  if lookup x old == Nothing then
    put ((x, 1) : old)
  else
    put (map (\ (y, n) -> if y == x then (y, n + 1) else (y, n)) old)
...
occurrences xs = do -- like squence_ but with state
  mapM_ count xs
  occ <- get
  return (length occ)
