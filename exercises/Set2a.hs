...
palindromify s = substring i (j + 1) s
    where
        (cl, cr) = findCenterPair (length s)
        (i, j) = findLongestPalindrome s cl cr

findCenterPair :: Int -> (Int, Int)
findCenterPair x
    | even x = (half_x - 1, half_x)
    | otherwise = (half_x - 1, half_x + 1)
    where
        half_x = div x 2

findLongestPalindrome :: String -> Int -> Int -> (Int, Int)
findLongestPalindrome s i j
    | i < 0 = (i + 1, j - 1)   -- end
    | (s !! i) /= (s !! j) = (i + 1, j - 1)
    | otherwise = findLongestPalindrome s (i - 1) (j + 1)
