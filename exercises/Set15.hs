...
statements xs ys = (\ x i y -> x ++ " is " ++ i ++ y) <$> xs <*> ["", "not "] <*> ys
...
recognizeOp :: String -> Maybe (Int -> Int)
recognizeOp "negate" = Just negate
recognizeOp "double" = Just (*2)
recognizeOp _ = Nothing

calculator :: String -> String -> Maybe Int
calculator op number = recognizeOp op <*> readMaybe number
...
validateDiv x y = div <$> pure x <*> check (y /= 0) "Division by zero!" y
...
validateAddress streetName streetNumber postCode =
  (\ name number code -> Address name number code)
  <$>
...
twoPersons name1 age1 employed1 name2 age2 employed2 =
  (\ name1 age1 employed1 name2 age2 employed2 -> [Person name1 age1 employed1, Person name2 age2 employed2])
  <$> name1 <*> age1 <*> employed1 <*> name2 <*> age2 <*> employed2
  -- we first define the function to create the target
  -- then lift it to the applicative context
...
boolOrInt s = checkBool <|> checkInt
  where
    checkBool = case readMaybe s of
      Just x -> pure (Left x)
      Nothing -> invalid "Not a Bool"
    checkInt = case readMaybe s of
      Just x -> pure (Right x)
...
normalizePhone s =
  check (length ss <= 10) "Too long" ss
  *>
  traverse (\ c -> check (isDigit c) ("Invalid character: " ++ [c]) c) ss
  where
    ss = filter (not . isSpace) s
...
parseExpression s =
  case ws of
    [a, op, b] ->
      parseOp op
      <*>
      (parseNumber a <|> parseVariable a)
      <*>
      (parseNumber b <|> parseVariable b)
  ...
  where
    ...
    parseVariable [c] = check (isAlpha c) ("Invalid variable: " ++ [c]) (Variable c)
...
f <#> x = myLiftA2 ($) f x  -- just apply f to x
...
myFmap f x = myLiftA2 (\ _ -> f) (myPure ()) x
