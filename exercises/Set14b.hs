...
-- Database
import Database.SQLite.Simple (open,execute,execute_,query,query_,Connection,Query(..),Only (Only),fromOnly)
...
balance db name = do
  res <- query db balanceQuery (Only name) :: IO [Only Int]
  return (sum (map fromOnly res))
...
parseCommand ts = parseCommand' (map T.unpack ts)

parseCommand' :: [String] -> Maybe Command
parseCommand' ["balance", account] = Just (Balance (T.pack account))
parseCommand' ["deposit", account, amount] = parseInt (T.pack amount) >>= \a -> Just (Deposit (T.pack account) a)
parseCommand' ["withdraw", account, amount] = parseInt (T.pack amount) >>= \a -> Just (Deposit (T.pack account) (-a))
...
perform db (Just cmd) = case cmd of
  (Deposit account amount) -> do
    deposit db account amount
    return (T.pack "OK")
  (Balance account) -> do
    balance db account >>= return . T.pack . show
