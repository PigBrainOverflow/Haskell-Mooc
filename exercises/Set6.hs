...
instance Eq a => Eq (List a) where
    ...
    Empty == Empty = True
    LNode xval xnode == LNode yval ynode = xval == yval && xnode == ynode
    _ == _ = False
...
instance Eq RationalNumber where
    ...
    RationalNumber xn xd == RationalNumber yn yd = xn * yd == yn * xd
...
instance Num RationalNumber where
    RationalNumber xn xd + RationalNumber yn yd = simplify $ RationalNumber (xn * yd + xd * yn) (xd * yd)
    RationalNumber xn xd * RationalNumber yn yd = simplify $ RationalNumber (xn * yn) (xd * yd)
    abs (RationalNumber xn xd) = RationalNumber (abs xn) xd
    signum (RationalNumber xn _) = RationalNumber (signum xn) 1
    fromInteger x = RationalNumber x 1
    negate (RationalNumber xn xd) = RationalNumber (-xn) xd
