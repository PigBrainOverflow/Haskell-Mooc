...
fmap2 function = fmap (fmap function)

fmap3 :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 function = fmap (fmap (fmap function))
...
instance Functor TwoList where
  fmap f (TwoNode x y xys) = TwoNode (f x) (f y) (fmap f xys)
  fmap _ _ = TwoEmpty
...
count target = foldr (\ x cnt -> if x == target then cnt + 1 else cnt) 0
...
inBoth xs = foldr f []
  where f x acc = if x `elem` xs then x : acc else acc
...
instance Foldable List where
  foldr f acc (LNode val ls) = f val $ foldr f acc ls
  foldr _ acc _ = acc
...
instance Foldable TwoList where
  foldr f acc (TwoNode x y xys) = f x (f y $ foldr f acc xys)
  foldr _ acc _ = acc
...
sumTree :: Monoid m => Tree m -> m
sumTree (Node val left right) = mappend (mappend (sumTree left) val) (sumTree right)
sumTree _ = mempty
