...
tick :: UpDown -> UpDown
tick (IncMode x) = IncMode (x + 1)
tick (DecMode x) = DecMode (x - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (IncMode x) = DecMode x
toggle (DecMode x) = IncMode x
...
toList Empty = []
toList (Pair key val keyvals) = (key, val) : toList keyvals

fromList :: [(k, v)] -> KeyVals k v
fromList [] = Empty
fromList ((key, val) : xs) = Pair key val $ fromList xs
...
fromBin :: Bin -> Int
fromBin b = fromBin' b 1 0

-- binary -> weight -> acc
fromBin' :: Bin -> Int -> Int -> Int
fromBin' End _ acc = acc
fromBin' (O b) w acc = fromBin' b (2 * w) acc
fromBin' (I b) w acc = fromBin' b (2 * w) (acc + w)
